...
class NewGoal extends ConsumerStatefulWidget {
  const NewGoal(
      {super.key,
      this.selectedGoal,
  ...
  @override
  ConsumerState<NewGoal> createState() {
    return _NewGoalState();
  }
}

class _NewGoalState extends ConsumerState<NewGoal> {
      ...
      if (enteredGoalDescription.text !=
          ...
          enteredStartDate.text !=
              DateFormat("MM/dd/yyyy")
                  .format(widget.selectedGoal![0].goalStartDate!) ||
          enteredEndDate.text !=
              DateFormat("MM/dd/yyyy")
                  .format(widget.selectedGoal![0].goalEndDate!)) {
        return true;
      }
    } else {
  ...
  bool checkifStatusUpdated() {
    if (widget.recordType == "Edit") {
      if (widget.selectedGoal![0].goalStatus!.isNotEmpty) {
        return true;
      }
    }
    return false;
  ...
  buildOnSubmit() {
    showDialog<String>(
      ...
      builder: (BuildContext context) => AlertDialog(
        ...
        content: Text(
          '''The record will be sent to the supervisor for approval. 
             Select 'Yes' to Confirm or 'No' to continue editing''',
          style: TextStyle(color: Theme.of(context).colorScheme.onSurface),
        ),
        actions: <Widget>[
          TextButton(
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (widget.recordType == "Edit") {
        ...
        requestBody = {
          ...
          'financialyear': widget.selectedGoal![0].financialyear,
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (_isLoading) {
              ...
              if (widget.recordType == "Edit") {
                final response = ref.watch(updateGoalMaster);
                ...
                final response = ref.watch(saveGoalMaster);
                return response.when(
                  loading: () => contentLoading,
                  error: (err, stack) => contentException('Error: $err'),
                  data: (config) => contentSuccess(response.value!),
