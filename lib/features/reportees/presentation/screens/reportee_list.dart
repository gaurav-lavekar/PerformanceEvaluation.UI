...
class ReporteeList extends ConsumerStatefulWidget {
  const ReporteeList({required this.onAdd, required this.onView, super.key});

  final void Function(bool flg, List<EmployeeMaster> selectedPeople,
      List<AssessmentMaster> selectedGoal, String selectedPage) onAdd;

  final void Function(
          bool flg, List<AssessmentMaster> selectedGoals, String selectedPage)
      onView;

  @override
  ConsumerState<ReporteeList> createState() => _ExpandableState();
}

class _ExpandableState extends ConsumerState<ReporteeList> {
  ...
  Future<void> initialData() async {
    var resp = await ref.read(assessmentMaster.future);
  }

  @override
  Widget build(BuildContext context) {
    ref.watch(employeeMasterbyId);
    List<EmployeeMaster> people =
        ref.watch(employeeMasterListNotifier).first.reportees!;
                            ...
                            IconButton(
                              onPressed: () {
                                ...
                                ref.watch(reporteeId.notifier).state =
                                    c.employeeId!;
                                widget.onView(
                                    true, appraisal, "EmployeeAssessmentGrid");
                              },
                              icon: const Icon(
                                Icons.visibility,
                                size: 20,
