...
class PerspectiveList extends ConsumerStatefulWidget {
  const PerspectiveList({required this.onAdd, super.key});

  final void Function(bool flg, List<PerspectiveMaster> selectedPerspective)
      onAdd;

  @override
  ConsumerState<PerspectiveList> createState() => _ExpandableState();
}

class _ExpandableState extends ConsumerState<PerspectiveList> {
  final moduleName = 'O_Perspective';
  List<PerspectiveMaster> bm = [];
  ...
  void removeItem(String id) async {
    try {
      ref.watch(deletePerspectiveBody.notifier).state = id;
  ...
  Widget build(BuildContext context) {
    ...
    List<PerspectiveMaster> perspectives =
        ref.read(perspectiveMasterListNotifier);
    List<PerspectiveMaster> searchList = [];

    if (_searchText != '') {
      var searchText = _searchText.toLowerCase();

      for (var c in perspectives) {
        if (c.goalPerspectiveName!.toLowerCase().contains(searchText)) {
          searchList.add(c);
        }
      }
      perspectives = searchList;
    }
                        ...
                        cells: [
                          ...
                          DataCell(Row(
                            children: [
                              IconButton(
                                onPressed: () {
                                  _buildOnEdit(c.goalPerspectiveId!);
                                },
                                icon: const Icon(
                                  Icons.edit_outlined,
                                  size: 20,
                              ...
                              AccessControlledWidget(
                                ...
                                child: IconButton(
                                  onPressed: () {
                                    _buildOnDelete(c.goalPerspectiveId!);
                                  },
                                  icon: const Icon(
    ...
    if (isDeleting) {
      ...
      final response = ref.watch(deletePerspectiveMaster);
  ...
  void _buildOnEdit(String id) {
    final perspectivelist =
        ref.watch(perspectiveMasterListNotifier).where((element) {
      return (element.goalPerspectiveId == id);
    }).toList();

    widget.onAdd(false, perspectivelist);
  }
          ...
          ElevatedButton(
            onPressed: () {
              try {
                ...
                if (perspectivelist.isNotEmpty) {
                  removeItem(perspectivelist[0].goalPerspectiveId!);
