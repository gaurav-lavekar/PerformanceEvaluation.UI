...
class FinancialYearList extends ConsumerStatefulWidget {
  const FinancialYearList({required this.onAdd, super.key});

  final void Function(bool flg, List<FinancialYearMaster> selectedFinancialYear)
      onAdd;

  @override
  ConsumerState<FinancialYearList> createState() => _ExpandableState();
}

class _ExpandableState extends ConsumerState<FinancialYearList> {
  final moduleName = 'O_FinancialYear';
  List<FinancialYearMaster> bm = [];
  ...
  void removeItem(String id) async {
    try {
      ref.watch(deleteYearBody.notifier).state = id;
  ...
  Widget build(BuildContext context) {
    ...
    List<FinancialYearMaster> financialyears = ref.read(yearMasterListNotifier);
    List<FinancialYearMaster> searchList = [];

    if (_searchText != '') {
      var searchText = _searchText.toLowerCase();

      for (var c in financialyears) {
        if (c.financialYear!.toLowerCase().contains(searchText)) {
          searchList.add(c);
        }
      }
      financialyears = searchList;
    }
                        ...
                        cells: [
                          ...
                          DataCell(Row(
                            children: [
                              IconButton(
                                onPressed: () {
                                  _buildOnEdit(c.financialyearid!);
                                },
                                icon: const Icon(
                                  Icons.edit_outlined,
                                  size: 20,
                              ...
                              AccessControlledWidget(
                                ...
                                child: IconButton(
                                  onPressed: () {
                                    _buildOnDelete(c.financialyearid!);
                                  },
                                  icon: const Icon(
    ...
    if (isDeleting) {
      ...
      final response = ref.watch(deleteFinancialYear);
  ...
  void _buildOnEdit(String id) {
    final financialyearlist =
        ref.watch(yearMasterListNotifier).where((element) {
      return (element.financialyearid == id);
    }).toList();

    widget.onAdd(false, financialyearlist);
  }
            ...
            onPressed: () {
              try {
                ...
                if (financialyearlist.isNotEmpty) {
                  removeItem(financialyearlist[0].financialyearid!);
