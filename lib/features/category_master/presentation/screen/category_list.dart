...
class CategoryList extends ConsumerStatefulWidget {
  const CategoryList({required this.onAdd, super.key});

  final void Function(bool flg, List<CategoryMaster> selectedCategory) onAdd;

  @override
  ConsumerState<CategoryList> createState() => _ExpandableState();
}

class _ExpandableState extends ConsumerState<CategoryList> {
  final moduleName = 'O_Category';
  List<CategoryMaster> bm = [];
  ...
  void removeItem(String id) async {
    try {
      ref.watch(deleteCategoryBody.notifier).state = id;
  ...
  Widget build(BuildContext context) {
    ...
    List<CategoryMaster> categories = ref.read(categoryMasterListNotifier);
    List<CategoryMaster> searchList = [];

    if (_searchText != '') {
      var searchText = _searchText.toLowerCase();

      for (var c in categories) {
        if (c.ratingCategoryName!.toLowerCase().contains(searchText)) {
          searchList.add(c);
        }
      }
              ...
              child: DataTable2(
                ...
                rows: categories
                    .map(
                      (c) => DataRow(
                        cells: [
                          DataCell(Text(c.ratingCategoryName!)),
                          DataCell(Row(
                            children: [
                              IconButton(
                                onPressed: () {
                                  _buildOnEdit(c.ratingCategoryId!);
                                },
                                icon: const Icon(
                                  Icons.edit_outlined,
                                  size: 20,
                              ...
                              AccessControlledWidget(
                                ...
                                child: IconButton(
                                  onPressed: () {
                                    _buildOnDelete(c.ratingCategoryId!);
                                  },
                                  icon: const Icon(
    ...
    if (isDeleting) {
      ...
      final response = ref.watch(deleteCategoryMaster);
  ...
  void _buildOnEdit(String id) {
    final categorylist = ref.watch(categoryMasterListNotifier).where((element) {
      return (element.ratingCategoryId == id);
    }).toList();

    widget.onAdd(false, categorylist);
  }
        ...
        actions: <Widget>[
          ...
          ElevatedButton(
            onPressed: () {
              try {
                final categorylist =
                    ref.watch(categoryMasterListNotifier).where((element) {
                  return (element.ratingCategoryId == id);
                }).toList();
                if (categorylist.isNotEmpty) {
                  removeItem(categorylist[0].ratingCategoryId!);
