...
class NewAssessment extends ConsumerStatefulWidget {
  const NewAssessment({
    super.key,
  ...
  final void Function(
    bool flg,
    List<AssessmentMaster> selectedAssessment,
  ) onAdd;

  @override
  ConsumerState<NewAssessment> createState() {
    return _NewAssessmentState();
  }
}

class _NewAssessmentState extends ConsumerState<NewAssessment> {
  ...
  String? selectedRating = "", overallrating = "", appraiserRating = "";
  ...
  List<List<TextEditingController>> measurementUnits = [];
  ...
  Future<void> initialData() async {
    ...
    if (widget.recordType == "Edit") {
      setState(() {
        selectedYear = widget.selectedAssessment![0].assessmentYear ?? "";

        selectedAppraisalType =
            widget.selectedAssessment![0].assessmentPeriod ?? "";
        ...
        assessmentDetails = widget.selectedAssessment![0].assessmentdetails!;
        ...
        goals = ref
            ...
            .where((goal) =>
                ...
                assessmentDetails.any(
                    (detail) => detail.goalsettingId == goal.goalSettingId))
            .toList();

        assessmentQualitatives = widget.selectedAssessment!.isEmpty
            ? []
            : widget.selectedAssessment![0].assessmentqualitatives!;

        for (int i = 0; i < assessmentDetails.length; i++) {
          enteredSelfRatings[assessmentDetails[i].goalsettingId!] =
              assessmentDetails[i].selfRating!;
          if (widget.selectedAssessment![0].assessmentStatus == "Approved") {
            enteredAppraiserRatings[assessmentDetails[i].goalsettingId!] =
                assessmentDetails[i].appraiserRating!;
          }
        }
      });
    }
  ...
  bool checkIfFormUpdated() {
    if (widget.recordType == 'Edit') {
      // if (enteredAppraiseeComment) {
      //   return true;
      // }
    } else {
      if (selectedYear!.isNotEmpty ||
          selectedAppraisalType!.isNotEmpty ||
          selectedQuarter!.isNotEmpty) return true;
    }
    return false;
  }
  ...
  buildOnSaveDraft() async {
    ...
    if (widget.recordType == "Edit") {
      if (!checkIfFormUpdated() &&
          status == widget.selectedAssessment![0].assessmentStatus) {
      ...
      if (widget.recordType == "Edit") {
        ...
        for (int i = 0; i < goals.length; i++) {
          assessmentDetails.add({
            ...
            'appraiseecomments': appraiseeComments[i][0].text.trim(),
            'actual': actualValues[i][0].text.trim(),
  ...
  onCallBackRating(String goalKey, String columnValue) {
    ...
    try {
      if (columnName == "Self Rating") {
        setState(() {
          enteredSelfRatings[goalKey] = int.parse(columnValue.substring(0, 1));
        });
      }
    } catch (e) {}
  }

  onCallBackQualitativeRating(String goalKey, String columnValue) {
    String columnName = "Qualtitative Rating";
    try {
      if (columnName == "Qualitative Rating") {
        setState(() {
          qualitativeItemScales[goalKey] =
              int.parse(columnValue.substring(0, 1));
        });
      }
    } catch (e) {}
  }
  ...
  onCallBackSupRating(String goalKey, String columnValue) {
    ...
    try {
      if (columnName == "Appraiser Rating") {
        setState(() {
          enteredAppraiserRatings[goalKey] =
              int.parse(columnValue.substring(0, 1));
        });
      }
    } catch (e) {}
  ...
  onCallBackOverallRating(String columnName, String columnValue) {
    try {
      if (columnName == "Overall Rating") {
        setState(() {
          overallrating = columnName.substring(0, 1);
        });
      }
    } catch (e) {}
  }

  Widget createContainer(List<GoalMaster> goals) {
    List<Widget> children = [];

    final assessment = widget.selectedAssessment;

    List<AssessmentDetails> assessmentDetails = assessment!.isEmpty
        ? []
        : widget.selectedAssessment![0].assessmentdetails!;
    ...
    Widget leadership = SizedBox(
      ...
      child: Padding(
        ...
        child: InputControl(
          isReadOnly: true,
          maxLine: 10,
          columnLabel: "Leadership Qualities",
    ...
    Widget overallRating = PickControl(
      ...
      itemlist: ref
          ...
          .map((element) =>
              '${element.ratingScale.toString()} - ${element.ratingCategory}')
          .toList(),
    ...
    // Widget qualitativeItemScale = SizedBox(
    //     width: 250,
    //     child: PickControl(
    //       isSearchPickList: true,
    //       columnLabel: "Qualitative Rating",
    ...
    if (widget.recordType == "Edit") {
      for (int i = 0; i < goals.length; i++) {
        ...
        selectedRating = assessmentDetails[i].selfRating == null
            ? ""
            : '''${assessmentDetails[i].selfRating.toString()} - ${ref.read(ratingMasterListNotifier).where((e) => e.ratingScale == assessmentDetails[i].selfRating).first.ratingCategory}''';
        goalDescriptions.add([
          TextEditingController(
              text: goals
                  .where((e) => e.goalSettingId == goalKey)
                  .first
                  .goalDescription)
        ]);
        perspectives.add([
        ...
        if (assessment.first.assessmentStatus == "Approved") {
          ...
          appraiserRating = assessmentDetails[i].appraiserRating == null
              ? ""
              : '${assessmentDetails[i].appraiserRating.toString()} - ${ref.read(ratingMasterListNotifier).where((e) => e.ratingScale == assessmentDetails[i].appraiserRating).first.ratingCategory}';
        }
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: Padding(
                          ...
                          child: InputControl(
                            isReadOnly: true,
                            columnLabel: "Measurement Unit",
                            columnEnteredValue: measurementUnits[i][0],
                          ),
                        ),
                      ),
                    ],
                      ...
                      children: [
                        ...
                        SizedBox(
                            ...
                            child: PickControl(
                              ...
                              columnSelectedValue: appraiserRating!,
                              itemlist: ref
                                  .read(ratingMasterListNotifier)
                                  .map((element) =>
                                      '${element.ratingScale.toString()} - ${element.ratingCategory}')
                                  .toList(),
                              onPickChange: (columnName, columnValue) =>
                                  onCallBackSupRating(goalKey, columnValue),
                              isMandatory: true,
                            )),
                      ],
                    ),
      ...
      if (goals.isNotEmpty) {
        for (int i = 0; i < goals.length; i++) {
          ...
          perspectives
              .add([TextEditingController(text: goals[i].goalPerspectiveName)]);
          targetValues
              .add([TextEditingController(text: goals[i].goalTargetValue)]);
          measurementUnits
              .add([TextEditingController(text: goals[i].goalMeasurementUnit)]);
