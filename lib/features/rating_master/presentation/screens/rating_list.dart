...
class RatingList extends ConsumerStatefulWidget {
  const RatingList({required this.onAdd, super.key});

  final void Function(bool flg, List<RatingMaster> selectedRating) onAdd;

  @override
  ConsumerState<RatingList> createState() => _ExpandableState();
}

class _ExpandableState extends ConsumerState<RatingList> {
  final moduleName = 'O_Rating';
  List<RatingMaster> bm = [];
  ...
  void removeItem(String id) async {
    try {
      ref.watch(deleteRatingBody.notifier).state = id;
  ...
  Widget build(BuildContext context) {
    ...
    ref.watch(ratingMaster);
    List<RatingMaster> ratings = ref.watch(ratingMasterListNotifier);
    List<RatingMaster> searchList = [];

    if (_searchText != '') {
      var searchText = _searchText.toLowerCase();

      for (var c in ratings) {
        if (c.ratingScale!.toString().contains(searchText) ||
            c.ratingCategory!.toLowerCase().contains(searchText)) {
          searchList.add(c);
        }
      }
      ratings = searchList;
              ...
              child: DataTable2(
                ...
                rows: ratings
                    .map(
                      (c) => DataRow(
                        cells: [
                          DataCell(Text(c.ratingScale!.toString())),
                          DataCell(Text(c.ratingCategory!)),
                          DataCell(Row(
                            children: [
                              IconButton(
                                onPressed: () {
                                  _buildOnEdit(c.ratingId!);
                                },
                                icon: const Icon(
                                  Icons.edit_outlined,
                                  size: 20,
    ...
    if (isDeleting) {
      ...
      final response = ref.watch(deleteRatingMaster);
  ...
  void _buildOnEdit(String id) {
    final ratinglist = ref.watch(ratingMasterListNotifier).where((element) {
      return (element.ratingId == id);
    }).toList();

    widget.onAdd(false, ratinglist);
  }
        ...
        actions: <Widget>[
          ...
          ElevatedButton(
            onPressed: () {
              try {
                final ratinglist =
                    ref.watch(ratingMasterListNotifier).where((element) {
                  return (element.ratingId == id);
                }).toList();
                if (ratinglist.isNotEmpty) {
                  removeItem(ratinglist[0].ratingId!);
